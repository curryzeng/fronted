<template>
          ...
          <Row>
           <Form  :model="detailForm" ref="detailForm" :rules="detailFormRules" :label-width="150">
              <Col
                ...
                :key="index"
                :md="{span: field.field_type_id === 55 ? 22 : 22}"
              >
 ...
 <!-- add by zhh -->
      <Col :md="{span: 22}" >
             <FormItem label="审核结果" >
               <div>实例名:{{insname}} 数据库名:{{dbname}}</div>
                  <Table border :columns="audit_result_cols" :data="audit_result_data"></Table>
             </FormItem>
      </Col>

      <Col :md="{span: 22}" >
             <FormItem label="执行结果"  v-if="exec_flag === 1">
               <div>实例名:{{insname}} 数据库名:{{dbname}}</div>
                  <Table border :columns="exec_result_cols" :data="exec_result_data"></Table>
             </FormItem>
 ...
 <!-- add by zhh -->
              ...
              <Col :md="{span: 22, offset:2}">
                <Button type="success" @click="handleTicketExec" :disabled="button_disabled_flag == 1 || exec_flag == 1"
                v-if="username == 'ops'">执行</Button>
                <Button
  ...
  methods: {
    init() {
      this.$store
        ...
        this.$store
          .dispatch("api_get_upload_result", { ticket_id: this.ticket_id })
          .then(resp => {
            this.audit_result_data = resp.data.audit_result
            this.exec_result_data = resp.data.exec_result
            this.exec_flag = resp.data.exec_flag
            this.insname = resp.data.insname
            this.dbname = resp.data.dbname;
          });
    ...
    handleTicketExec(){
      ...
      this.button_disabled_flag = 1
      this.$store.dispatch('api_exec_upload', {ticket_id:ticket_id}).then(resp => {
        this.exec_result_data = resp.data.exec_data
        this.exec_flag = 1
       // location.reload()
      }).catch(error => {
        console.log('调用exec_upload接口失败')
      })
    }
  },
  computed: {
