      ...
      <Card style="text-align: left;">
        ...
        <div class="form" v-if="!fieldEmpty">
          <Form ref="newForm" :model="newForm" :rules="newFormRules" :label-width="150">
            <Row>
              <Col :md="{span: item.field_type_id === 55 ? 22 : 22}" v-for="(item, index) in init_state.field_list" :key="index">
            ...
            <!--  add by zhh -->
             ...
             <Row>
                 <FormItem label="请选择数据库">
                   <Select v-model="autit_upload_data.dbid" filterable clearable style="width:400px">
                       <Option v-for="item in all_db_list" :value="item.value" :key="item.value">{{ item.label }}</Option>
                    </Select>
                 </FormItem>
                 ...
                 <FormItem label="请上传SQL文件">
                  ...
                  <FormItem label="审核结果" v-if="audit_result_data">
                      <Table border :columns="audit_result_cols" :data="audit_result_data"></Table>
                   </FormItem>
              </Row>
              <p>{{autit_upload_data.dbid}}</p>
              <p>{{username}}</p>
...
export default {
  ...
  data () {
    return {
      ...
      audit_result_cols: [
                          {title: 'SQL序号',key: 'order_id'},
                          {title: '影响行数',key: 'affected_rows'},
                          {title: '审核结果',key: 'stage_status'},
                          {title: '错误级别',key: 'error_level'},
                          {title: '错误消息',key: 'error_message'},
                          {title: 'SQL语句',key: 'sql'}
                      ],
      audit_result_data:[],
      all_db_list:[],
      autit_upload_data:{dbid:0, username:''}
    }
  },
  methods: {
    init () {
      ...
      this.autit_upload_data.username = this.username
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          this.$store.dispatch('api_post_ticket', data).then(resp => {
            this.commit_upload_data.resp_ticket_id = resp.data.data.ticket_id
            console.log('ticket_id is ', resp.data.data.ticket_id)
            this.do_commit_upload(this.commit_upload_data)
    ...
    afterupload(response, file, fileList){
      ...
      this.commit_upload_data.uniq_file_name=response.uniq_file_name
      this.audit_result_data = response.audit_data
    },
    do_commit_upload(data){
      this.$store.dispatch('api_commit_upload', data).then(resp => {
      }).catch(error => {
        console.log('调用commit_upload接口失败')
      })
    },
    get_all_db_list(){
